...
ct_2016$new_town[164] <-  "Windsor Locks"
  ...
  select(new_town, percent_afford) %>% 
   mutate(year = 2016,
          percent_afford = percent_afford %>% str_replace("1.300/0", "1.30") %>% 
            ...
            exp_status = case_when(percent_afford > 10 ~ "Exempt", TRUE ~ "Non-Exempt")) %>% 
...
```{r integrity}
#2018
ct_df$town[which(ct_df$town == "North" & ct_df$year == 2018)] <- "North Stonington"
#2017
ct_df$percent_afford[which(ct_df$town == "Windsor" & ct_df$exmp_status == "Exempt" & ct_df$year == 2012)] <- 9.15
#2012
ct_df$town[which(ct_df$town == "Windsor" & ct_df$exmp_status == "Exempt")] <- "Windsor Locks"
ct_df$percent_afford[which(ct_df$town == "Windsor" & ct_df$exmp_status == "Exempt" & ct_df$year == 2012)] <- 9.15
ct_df$town[which(ct_df$town == "North" & ct_df$percent_afford == 2.20)] <- "North Branford"
ct_df$town[which(ct_df$town == "North" & ct_df$percent_afford == 0.95)] <- "North Stonington"
#2013
ct_df$town[which(ct_df$town == "Windsor" & ct_df$exmp_status == "Exempt")] <- "Windsor Locks"
ct_df$percent_afford[which(ct_df$town == "Windsor" & ct_df$exmp_status == "Exempt" & ct_df$year == 2013)] <- 9.25
ct_df$town[which(ct_df$town == "North" & ct_df$percent_afford == 2.33)] <- "North Branford"
ct_df$town[which(ct_df$town == "North" & ct_df$percent_afford == 1.26)] <- "North Stonington"
```
...
ct_df <- ct_df %>% bind_rows(ct_appeals_2010) %>% bind_rows(ct_2016)

#ct_df %>% filter(year == 2011) %>% pull(town) %>% setdiff(ct_towns,.) 

ct_df <- ct_df %>% 
  add_row(town = "Beacon Falls", percent_afford = 1.24, year = 2011, exmp_status = "Non-Exempt") %>% 
  add_row(town = "Old Lyme", percent_afford = 1.37, year = 2011, exmp_status = "Non-Exempt") %>% 
  add_row(town ="Old Saybrook", percent_afford = 1.27, year = 2011, exmp_status = "Non-Exempt")

ct_df$town[ct_df$town=="Windsor" & ct_df$percent_afford > 8 ] <- "Windsor Locks"
```
